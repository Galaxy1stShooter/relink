...
bool is_waypoint_received = false;
...
void rcvInitPosCallback(const message_files::PoseStampedArrayConstPtr& msg)
{
    ...
    {
        if (uav_id != pose.header.seq) continue;
        
        if (pose.pose.position.z < 0.0)
        {
          ROS_ERROR("[simulator of uav%d] Initial position underground!", uav_id);
        }
        else
        {
          position = Eigen::Vector3d(pose.pose.position.x,
                                     pose.pose.position.y,
                                     pose.pose.position.z);
        }
        ROS_INFO("[swarm] UAV %d initial position set to:", uav_id);
...
void rcvWaypointCallback(const nav_msgs::PathConstPtr& msg)
{

  is_waypoint_received = true;
...
main(int argc, char** argv)
{
  ...
  ros::Subscriber m_sub =
    n.subscribe("moment_disturbance", 100, &moment_disturbance_callback,
                ros::TransportHints().tcpNoDelay());
  ros::Subscriber init_pos_sub = n.subscribe("initial_pos", 10, rcvInitPosCallback);
  ros::Subscriber waypoint_sub = nh.subscribe("waypoint_generator/waypoints", 1, rcvWaypointCallback);
  ...
  */
  ...
  while (n.ok())
  {
    ...
    if (is_waypoint_received) quad.step(dt);
