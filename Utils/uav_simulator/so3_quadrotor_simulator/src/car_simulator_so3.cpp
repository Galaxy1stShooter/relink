...
{
  ros::init(argc, argv, "car_simulator_so3");
  ...
  double                        _init_x, _init_y, _init_z, _init_theta;
  ...
  n.param("simulator/init_state_theta", _init_theta, 0.0);

  Eigen::Vector3d position = Eigen::Vector3d(_init_x, _init_y, _init_z);
  Eigen::Matrix3d R;
  R <<  cos(_init_theta * M_PI / 180), sin(_init_theta * M_PI / 180), 0.0, 
       -sin(_init_theta * M_PI / 180), cos(_init_theta * M_PI / 180), 0.0, 
       0.0                           , 0.0                          , 1.0;
  QuadrotorSimulator::Quadrotor::State init_state;
  init_state.x = position;
  init_state.v = Eigen::Vector3d::Zero();
  init_state.R = R;
  init_state.omega     = Eigen::Vector3d::Zero();
  init_state.motor_rpm = Eigen::Array4d::Zero();
