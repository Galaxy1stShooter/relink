...
// close points checking needed
Eigen::Vector3d pt_prev;
double close_thres;

// publish free waypoints needed
int point_num;
std::deque<geometry_msgs::PoseStamped> waypoint_queue;
...
void publish_waypoints() {
    ...
    ROS_INFO("publish uav%d, x: %f, y: %f", uav_id, waypoints.poses[0].pose.position.x, waypoints.poses[0].pose.position.y);
...
/*    if (!is_odom_ready) {
    ...
    ROS_ERROR("[waypoint_generator] Triggered111!");
    ...
    if (waypoint_type == string("circle")) {
        ...
        ROS_ERROR("[waypoint_generator] Triggered!");
        geometry_msgs::PoseStamped pt = waypoint_queue.front();
...
bool isNear(const Eigen::Vector3d& pt_1, const geometry_msgs::PoseStamped& pt_2)
{
    Eigen::Vector3d tmp;
    tmp << pt_2.pose.position.x, pt_2.pose.position.y, pt_2.pose.position.z;

    return (tmp - pt_1).norm() < close_thres;
}


void rcvSolutionCallback(const message_files::PoseStampedArrayConstPtr& msg)
{
    ...
    {
        if (uav_id == pose.header.seq)
        {
            if (pose.pose.position.z < 0.0)
            {
                ROS_ERROR("[waypoint_generator of uav%d] Goal position underground!", uav_id);
                continue;
            }

            geometry_msgs::PoseStamped pt = pose;
            if (isNear(pt_prev, pt))
            {
                ROS_INFO("[waypoint_generator of uav%d] Ignore the waypoint!", uav_id);
                continue;
            }
            ...
            publish_waypoints_vis();
            publish_waypoints();
            pt_prev(0) = pt.pose.position.x;
            pt_prev(1) = pt.pose.position.y;
            pt_prev(2) = pt.pose.position.z;
        }
    }
...
int main(int argc, char** argv) {
    ...
    ros::NodeHandle n("~");
    n.param("waypoint_type", waypoint_type, string("manual"));
    n.param("close_thres", close_thres, -1.0);
    // ROS_ERROR("[waypoint_generator] Triggered!");

    n.param("point_num", point_num, 1);
    for (size_t i = 0; i < 3; i++)
    {
        geometry_msgs::PoseStamped pt;
        n.param("point" + std::to_string(i) + "_x", pt.pose.position.x, -1.0);
        n.param("point" + std::to_string(i) + "_y", pt.pose.position.y, -1.0);
        n.param("point" + std::to_string(i) + "_z", pt.pose.position.z, -1.0);
        ROS_ERROR("publish uav, x: %f, y: %f", pt.pose.position.x, pt.pose.position.y);
        waypoint_queue.push_back(pt);
    }
    ...
    ros::Subscriber solution_sub = n.subscribe("/optimizer/solution", 10, rcvSolutionCallback);
    pub1 = n.advertise<nav_msgs::Path>("waypoints", 50);
    pub2 = n.advertise<geometry_msgs::PoseArray>("waypoints_vis", 10);
    pt_prev << 10000.0, 10000.0, 10000.0;
