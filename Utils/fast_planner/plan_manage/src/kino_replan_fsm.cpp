...
void KinoReplanFSM::init(ros::NodeHandle& nh) {
  ...
  reach_pub_   = nh.advertise<std_msgs::Header>("/planning/reach", 10);
}
...
void KinoReplanFSM::execFSMCallback(const ros::TimerEvent& e) {
  ...
  switch (exec_state_) {
    ...
    case WAIT_TARGET: {
      if (!have_target_)
      {
        std_msgs::Header reach_msg;
        reach_msg.seq = uav_id_;
        reach_pub_.publish(reach_msg);
        return;
      }
      else {
